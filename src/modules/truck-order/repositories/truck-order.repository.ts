...
export class TruckOrderRepository {
  ...
  async findAll(searchFilter: unknown, offset: number, limit: number) {
    const truckOrders = await this.truckOrderModel.find(searchFilter).lean().sort({ createdAt: 'desc' }).skip(offset).limit(limit).populate({ path: 'truckId', populate: { path: 'productId' } }).populate({ path: 'buyerId', populate: { path: 'userId' } })

    const populatedTrucks = await Promise.all(truckOrders.map(async (truck) => {
      if (truck.profileType === 'Transporter') {
        const transporter = await this.transporterModel.findById(truck.profileId).lean();
        return { ...truck, profileId: transporter };
      } else if (truck.profileType === 'Seller') {
        const seller = await this.sellerModel.findById(truck.profileId).lean();
  ...
  async findOne(orderId: string) {
    ...
    const truckOrder = await this.truckOrderModel
      .findById(orderId)
      .populate({ path: 'truckId', populate: [{ path: 'depotHubId' }, { path: 'productId' }] })
      .populate({
        path: 'buyerId',
    ...
    if (truckOrder.profileType === 'Transporter') {
      const transporter = await this.transporterModel.findById(truckOrder.profileId).populate('userId').lean();
      return { ...truckOrder, profileId: transporter };
    } else if (truckOrder.profileType === 'Seller') {
      const seller = await this.sellerModel.findById(truckOrder.profileId).populate('userId').lean();
