...
export class ProductUploadService {
  ...
  async saveNewProductUploadData(productUploadData: ProductUploadDto, user: IJwtPayload) {
    if (user.role !== 'seller') throw new ForbiddenException("You are not authorized to make this request")
    const seller = await this.sellerRepository.findOneQuery({ userId: user.id });
    const product = await this.productRepository.findOne(productUploadData.productId);
    const depotHub = await this.depotHubRepository.findOne(productUploadData.depotHubId);
  ...
  async getAllProductUploads(query: ProductUploadQueryDto) {
    const { page = 1, limit = 10, productId, sellerId, depotHubId, volume, status } = query;
    let offset = 0;
    if (page && page > 0) {
      offset = (page - 1) * limit;
    }
    ...
    if (volume) searchFilter.$and.push({ volume: { $gte: volume } });
    if (status) searchFilter.$and.push({ status });

    if (!searchFilter.$and.length) delete searchFilter.$and;

    const productUploads = await this.productUploadRepository.findAll(searchFilter, offset, limit);
    const total = await this.productUploadRepository.getTotalProductUploads(searchFilter);
  ...
  async getProductUploadDetail(productUploadId: string) {
    const productUpload = await this.productUploadRepository.findOne(productUploadId);
  ...
  async updateProductUploadData(productUploadId: string, productUploadData: ProductUploadDto, user: IJwtPayload) {
    if (user.role !== 'seller') throw new ForbiddenException("You are not authorized to make this request")
    const seller = await this.sellerRepository.findOneQuery({ userId: user.id });
    const product = await this.productRepository.findOne(productUploadData.productId);
    const depotHub = await this.depotHubRepository.findOne(productUploadData.depotHubId);
    ...
    productUploadData.productId = product._id;
    productUploadData.depotHubId = depotHub._id;
    return await this.productUploadRepository.update(productUploadId, productUploadData);
  }

  async deleteProductUploadData(productUploadId: string) {
    const productUpload = await this.productUploadRepository.delete(productUploadId);
    if (!productUpload) {
      throw new NotFoundException({
        message: 'Product upload not found',
        statusCode: 404
