...
export class ProductUploadRepository {
  ...
  async findOne(productUploadId: string | Types.ObjectId) {
    ...
    return await this.productUploadModel.findById(productUploadId).populate('productId').populate('depotHubId').populate('sellerId').exec();
  }

  async update(productUploadId: string, productUploadData: ProductUploadDto) {
    try {
      const updatedProductUpload = await this.productUploadModel.findByIdAndUpdate(
        productUploadId,
        productUploadData,
      ...
      if (!updatedProductUpload) {
        throw new NotFoundException(`ProductUpload with ID ${productUploadId} not found.`);
      }
  ...
  async delete(productUploadId: string) {
    if (!isValidObjectId(productUploadId)) return null
    const productUpload = await this.productUploadModel.findByIdAndDelete(productUploadId)
